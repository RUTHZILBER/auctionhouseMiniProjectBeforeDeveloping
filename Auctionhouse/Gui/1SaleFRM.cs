...
{
    public partial class SaleFRM1 : Form
    {
        private SaleDB tbls;
        private Sale s;
        private DateTime d1;
        ...
        public SaleFRM1(string key)//פעולה בונה ב עבור החזרה מהחפוש
        {
            InitializeComponent();
            tbls = new SaleDB();
            tblso = new Sale_offerDB();
        ...
        private bool CreateFields(Sale s)
        {
            ...
            try
            {
                s.Product_code = Convert.ToInt32(cmbProduct_code.SelectedValue.ToString());
            }
            catch (Exception ex)
            {
                errorProvider1.SetError(txtSale_code, ex.Message);
